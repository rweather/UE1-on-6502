;
; Divide a 8-bit number by a 4-bit number to give a 8-bit quotient and a
; 4-bit remainder.
;
; by Rhys Weatherley
;
...
;      IR7 is also loaded, as the MSB of the 8-bit dividend.
;   2. Resume and then the program halts again waiting for the low 7 bits
;      of the dividend to be entered on IR7..IR1.
;   3. Resume and the quotient will eventually be written to OR7..OR0.
;      The remainder will be in SR3, SR2, SR1, SR0.
;
; If division by zero occurs, then all of OR7..OR0 will be set to 1.
; That is a quotient of 255, which is the closest the program can get to
...
STOC SR7
;
; Prompt for the 4-bit divisor and the top-most bit of the dividend
...
;
; Read the top-most bit of the dividend from IR7 and put it in SR4.
;
LD   IR4
STO  SR3
LD   IR3
...
STO  SR4
;
; Prompt for the low 7 bits of the dividend on the input switches.
;
IOC  SR0
NOPF SR0
NOP0 SR0        ; NOP0's to wait for the motor to spin down.
...
NOP0 SR0 
;
; The dividend is currently in SR4, IR7..IR1 (MSB to LSB).  We leave most of
; it on the input switches, effectively using them as 7 bits of extra RAM.
;
; Note: Changing the input switches while the remaining code runs will
; modify the result in unexpected ways.  Don't do that!
;
; Round 0, quotient result bit in OR7.
;
; Working register W = 0, SR7, SR6, SR5, SR4.  The highest bit of the
; dividend has already been shifted into SR4.
;
...
LD   SR7
SUB  SR3
XOR  RR         ; Shift CAR into RR.
ADD  RR
STO  OR7        ; Set the quotient bit for this round.
;
